...
export default function PeopleQueue({ beforeUser, afterUser, userTapped, corridor, stopName, boardedCount=0 }) {
  ...
  const total=Math.max(0,original.length-boardedCount),size=personSize(total);
  ...
  return <section className="rounded-2xl border p-4" style={{background:C.card,borderColor:C.border}}>
    ...
    {userTapped&&total>0&&<p className="mt-2 text-[11px] font-semibold" style={{color:C.userBlue}}>{boardedCount>0?"Boarding in progress — you are still counted":"The blue person is you — position 12 in the queue"}</p>}
    {total===0&&<p className="mt-2 text-[11px] font-extrabold" style={{color:C.success}}>All 16 waiting students have boarded</p>}
  </section>;
}
