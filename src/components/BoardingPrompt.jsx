...
export default function BoardingPrompt({ show, onBoarded, onMissed, bus, stop }) {
  ...
  return <section className="rounded-2xl border-2 p-4 text-center transition-all duration-[400ms]" style={{ background:C.card,borderColor:C.primaryLight }}>
    ...
    {!answer?<div className="mt-3 grid grid-cols-2 gap-2"><button onClick={()=>respond("boarded")} className="rounded-xl px-2 py-3 text-[11px] font-extrabold text-white" style={{background:C.primary}}>✓ Yes, I'm on</button><button onClick={()=>respond("missed")} className="rounded-xl border px-2 py-3 text-[11px] font-extrabold" style={{background:C.bg,borderColor:C.border,color:C.text}}>✕ No, I missed it</button></div>:<p className="mt-4 font-extrabold" style={{color:answer==="boarded"?C.success:C.primary}}>{answer==="boarded"?"Have a safe trip! 👋":`You're still in the queue. The next bus is about 24 minutes away.`}</p>}
    <p className="mt-3 text-[9px]" style={{color:C.textMuted}}>If you do not answer, you will leave the queue after 60 seconds</p>
  </section>;
}
