...
export default function GPSPermission({ selectStop }) {
  ...
  const allow=()=>{debugLog("gps","Simulated detection started",{delay:GPS_DETECTION_DELAY});setLoading(true);setTimeout(()=>{debugLog("gps","KDSE detected");selectStop(STOPS.find(s=>s.id==="kdse"),true);navigate("/destination")},GPS_DETECTION_DELAY)};
