...
it("navigates all ten steps forward and backward five times without drift",()=>{render(<App/>);const next=screen.getByRole("button",{name:"Next step"}),previous=screen.getByRole("button",{name:"Previous step"});for(let run=0;run<5;run+=1){for(let step=1;step<10;step+=1)fireEvent.click(next);expect(screen.getByText("7:57 · Both overflow queues are clear")).toBeInTheDocument();expect(screen.getByText("Overflow cleared")).toBeInTheDocument();expect(screen.getAllByText("5 updates")).toHaveLength(2);expect(within(screen.getByLabelText("Stop KDSE")).getByText("0")).toBeInTheDocument();for(let step=9;step>0;step-=1)fireEvent.click(previous);expect(screen.getByText("7:40 · E1 boards at KDOJ")).toBeInTheDocument();expect(screen.getByText(/Press the right arrow/)).toBeInTheDocument();}});
...
it("combines evaluation and safety checks into one decision",()=>{render(<App/>);const next=screen.getByRole("button",{name:"Next step"});for(let step=1;step<=4;step+=1)fireEvent.click(next);expect(screen.queryByText(/Checking earlier departure/)).not.toBeInTheDocument();fireEvent.click(next);expect(screen.getByText("Earlier departure approved")).toBeInTheDocument();expect(screen.getByText("Bus E2 will leave at 7:50 instead of 8:00 · All 6 safety checks passed, including a safe 10-minute gap behind E1")).toBeInTheDocument();expect(screen.queryByText("SAME ROUTE ✓")).not.toBeInTheDocument();});
