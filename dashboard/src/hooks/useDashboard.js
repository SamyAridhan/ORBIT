...
export function useDashboard(){
  ...
  const state=useMemo(()=>{
    ...
    return {...canonical,buses:canonical.buses.map(bus=>bus.id===override?{...bus,status:"COMMUTING",etaGain:0}:bus),logs:[{type:"REJECT",agent:`FLEET_MANAGER`,event:"TIMING_OVERRIDE_CONFIRMED",desc:`Bus ${override} returned to planned timing · admin/override simulated`,chips:[],key:`override-${override}`,step},...canonical.logs]};
